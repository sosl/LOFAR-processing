...
lcucommand = "ssh glow609 ssh de609c killpointing"
...
lcucommand = "ssh glow609 ssh de609c beamctl --antennaset=HBA_JOINED --rcus=0:191 --rcumode=5 --beamlets=0:365 --subbands=93:458 --anadir=2.094,0.98,AZELGEO --digdir=2.094,0.98,AZELGEO&"
if options.Verbose:
...
lofar1command = "ssh lofarD3 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + " " + str(inttime)  + " " + data_dir
lofar1command += " " + starttime + " DE609 1 1"
# the last thre arguments there are: station lane verbosity
lofar1command += " >> ~/PSR_Logs/LuMP_"+data_dir+"_"+Pulsar+"_lane1.log 2>&1' "

lofar2command = "ssh lofarD4 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + "  " + str(inttime)  + " " + data_dir
lofar2command += " " + starttime + " DE609 2 1"
lofar2command += " >> ~/PSR_Logs/LuMP_"+data_dir+"_"+Pulsar+"_lane2.log 2>&1' "

lofar3command = "ssh lofarD4 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + " " + str(inttime)  + " " + data_dir
lofar3command += " " + starttime + " DE609 3 1"
...
lcucommand = "ssh glow609 ssh de609c killpointing " # + Pulsar
lcuproc = os.popen(lcucommand)
