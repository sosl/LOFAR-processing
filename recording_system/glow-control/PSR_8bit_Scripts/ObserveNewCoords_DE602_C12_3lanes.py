...
if options.Verbose:
    ...
    print " - ", lcucommand
lcuproc = False
psrcat_top='/home/observer/PSR_files/psrcat/psrcat_tar/'
os.environ["PSRCAT_RUNDIR"] = psrcat_top
os.environ["PSRCAT_FILE"] = psrcat_top + "psrcat.db"
psrcat_command= psrcat_top + 'psrcat -all -o short -nohead -nonumber -c "raj decj" ' + Pulsar
print psrcat_command
psrcatproc = sb.Popen(psrcat_command, shell=True, stdout=sb.PIPE)
print "run psrcat"
for line in psrcatproc.stdout:
    print line
exit(1)
...
lcucommand = "ssh glow602 ssh de602c /data/home/user9/LCU-scripts/PSR_8bit_Scripts/observe-coord-3lane.csh " + Pulsar
if options.Verbose:
...
lofar1command = "ssh lofarC1 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + " " + str(inttime)  + " " + data_dir
...
lofar2command = "ssh lofarC2 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + "  " + str(inttime)  + " " + data_dir
lofar2command += " " + starttime + " DE602 2 1"
lofar2command += " >> ~/PSR_Logs/LuMP_"+data_dir+"_"+Pulsar+"_lane2.log 2>&1' "

lofar3command = "ssh lofarC2 '~/PSR_8bit_Scripts/RunLuMP_universal_3quarters.sh " + Pulsar + " " + str(inttime)  + " " + data_dir
