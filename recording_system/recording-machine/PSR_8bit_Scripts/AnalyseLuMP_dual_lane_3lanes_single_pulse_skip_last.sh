...
then
    ...
    echo "e.g.: $0 STATION B0329+54 [THREADS] [CHANNELS] [SP_THREDS]"
    exit
elif [ $# -eq 2 ]
then
    station=$1
...
do
  ...
  do
      ...
      if  [ $small_DM -eq 1 ]
      then
	  # estimate duration of data:
	  length_in_sec=$(ls -l ${file} | awk '{print ($5-4096)/122*0.00000128}')
	  length_in_sec_per_thread=$(echo $length_in_sec | awk '{print int(($1+1)/'$threads_single')}')
	  mkdir -p single_pulses_${lane}
	  cd single_pulses_${lane}
	  for thread_id in $(seq 1 $threads_single)
	  do
		  skip_now=$(echo '('$thread_id'-1)'*${length_in_sec_per_thread}-60 | bc)
		  if [[ $skip_now -lt 0 ]] 
		  then
			  skip_now=0
			  length_in_sec_per_thread_now=${length_in_sec_per_thread}
		  else
			  length_in_sec_per_thread_now=$(echo ${length_in_sec_per_thread}+60 |bc)
		  fi
		  if [[ $thread_id -eq $threads_single ]]
		  then
			  length_in_sec_per_thread_now=$(echo ${length_in_sec_per_thread_now}-60 | bc)
		  fi
		  time dspsr -F ${channels}:D -E ../eph.par -W -2c0 -sK -T ${length_in_sec_per_thread_now} -S ${skip_now} -E ../eph.par -minram 1 ../$file $fft_bench&
	  done
	  ...
	  # estimate duration of data:
	  length_in_sec=$(ls -l ${file} | awk '{print ($5-4096)/122*0.00000128}')
	  length_in_sec_per_thread=$(echo $length_in_sec | awk '{print int(($1+1)/'$threads')}')
	  mkdir -p single_pulses_${lane}
	  cd single_pulses_${lane}
	  for thread_id in $(seq 1 $high_dm_threads)
	  do
		  skip_now=$(echo '('$thread_id'-1)'*${length_in_sec_per_thread}-60 | bc)
		  if [[ $skip_now -lt 0 ]] 
		  then
		  ...
		  #last thread needs to skip some data or else sometimes we are stuffed. If the number of seconds is even one sample off from the acutal number of samples left, dspsr will just fail
		  if [[ $thread_id -eq $high_dm_threads ]]
		  then
			  length_in_sec_per_thread_now=$(echo ${length_in_sec_per_thread_now}-60 | bc)
		  fi
		  time dspsr -F ${channels}:D -E ../eph.par -W -2c0 -sK -T ${length_in_sec_per_thread_now} -S ${skip_now} -E ../eph.par -U minX2 -minram 1 ../$file $fft_bench&
	  done
	  cd -
