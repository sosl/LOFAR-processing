...
then
    ...
    echo "e.g.: RunLuMP.csh B0329+54 [threads] [channels]"
    exit
elif [ $# -eq 2 ]
then
    station=$1
...
if [[ $RA == "" ]] || [[ $DEC == "" ]]
then
    RA=`psrcat -all -c RAJ -x $PSR | awk '{print $1}'`
    DEC=`psrcat -all -c DECJ -x $PSR | awk '{print $1}'`
fi
...
do
  ...
  do
      ...
      then
          ...
          dspsr -F ${channels}:D -A -E eph.par -L 10 -t $threads -minram 1 -O ${PSR}.${epoch}.lofar${lane}${extra} $file ${fft_bench}
      else
          echo "High DM. Slowing down..."
          dspsr -F ${channels}:D -A -E eph.par -L 10 -U minX2 -t $high_dm_threads -minram 1 -O ${PSR}.${epoch}.lofar${lane}${extra} $file ${fft_bench}
      fi
        #mv ${file}.ar.ar SubBands_${lane}/
      mv $file ToThrow_${lane}/
      prefix=`basename $file .raw`
      mv ${prefix}* ${PSR}_${lane}.info ToThrow_${lane}/
      mkdir -p Logs_${lane}
      mv *log Logs_${lane}
  ...
  then
	  ...
	  mv *_${lane}\.00*lis *_${lane}\.00*valid ToThrow_${lane}
  fi
