...
if [[ "$PSR" == "J0837+0610" ]] || [[ "$PSR" == "B1919+21" ]] || [[ "$PSR" == "B1508+55" ]] || [[ "$PSR" == "B2117+47" ]] || [[ "$PSR" == "B0329+54" ]]
then
	LuMP_version=0
else
	LuMP_version=1
...
lane=$6
if [[ $lane -gt 3 || $lane -lt 1 ]]
then
	echo lane needs to be between 1 and 3
	exit 16
fi
# set up auxiliary per-lane information
beamlet_start=$(( ($lane-1)*122 ))
beamlet_end=$(( ($lane)*122 ))
subband_start=$(( ($lane-1)*122 + 93 ))
subband_end=$(( ($lane)*122 + 93 ))
...
cd $PSR
    ...
    Basic_LuMP_Recorder.py --port=${port} --clock_speed=200 --beamlets_per_lane=122 --datadir=${datadir} --data_type_in=L_intComplex16_t --station_name=${station} --writer_type=LuMP${LuMP_version} --physical_beamlet_array='['${beamlet_start}':'${beamlet_end}']' --rcumode_array='[5]*122' --epoch_array='[J2000]*122' --verbose --duration=${TINT} --subband_array='['${subband_start}':'${subband_end}']' --filename_base=${PSR}_${lane} --sourcename_array="[$PSR]*122" --rightascension_array="[$RAJR]*122" --declination_array="[$DECJR]*122" --start_date=${starttime} --recorder_num_cores=2
